/*--------------------------------------------------------------------------------------
  ...
  snake movements as inputs and returns the direction the player has inputted. This module provides
  the main delay between each movement of the snake.
    
  --------------------------------------------------------------------------------------

  Debouncing is done through a shift and add method. Whenever the detected input is active the 
  corresponding register is shifted left one and a 1 added to it effectively placing a 1 on the
  right end. Whenever an input is detected as not active, the register is returned to zero.

  The reason for this method is that it is incredibly easy to detect a rising edge while debouncing
  an input. If the function required five consecutive active reads to detect the input, the
  register would be 0b00011111 at the earliest possible time and an output can be given from this
  read.

  The difference between this method and using an integer to count is that an integer can overflow
  and would give a second input if the button was held down for sufficiently long. The shift and
  add method will read 0b11111111 when the button is held down and would not return an additional
  input unless the button was released.
...
// Interval and total time for debouncing can both be changed, but total_time should be a multiple of interval.
// For debounce mask purposes, total_time should be longer than interval but not over 16 times larger.
...
/*--------------------------------------------------------------------------------------
  Variables
  --------------------------------------------------------------------------------------*/
  // Added as a variable rather than a constant to prevent calculation being completed on every call.
...
/*--------------------------------------------------------------------------------------
  input_init()
    ...
    Can be called from the start of Snake game function or during setup phase of microcontroller.
  --------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------
  get_direction_input()
    Descriptions:
      Recieves user input through a debouncer and checks if it is a valid direction for snake to travel
      before returning the direction to the main loop. This function also performs the main delay between
      each move of the snake.
    Inputs:
...
{
  // Variable new_input keeps track of the newly inputted direction while the player_data variable keeps track of the old direction.
  // Old direction is retained to guarantee that multiple valid inputs in one cycle will not allow snake to go backwards.
  snake_direction new_input = player_data->player_direction;
  // Temp value used to store debounced values and check valid direction before temporarily storing.
  snake_direction debounced_input = NONE;

  // Total delay between snake movements is variable, but time between each debouncing check must be 5ms (DEBOUNCING_INTERVAL).
  ...
  for ( int16_t i = 0; (input_wait / DEBOUNCING_INTERVAL) > i; i++)
  {
    ...
    if ((NONE != debounced_input) && is_not_opposite_direction( player_data->player_direction, debounced_input))
    {
  ...
  // If delay is <5ms debouncing will not occur and game will be unplayable, but with the snake moving once every 5ms the game is already unplayable for humans.
...
/*--------------------------------------------------------------------------------------
  receive_debounced_input()
    Descriptions:
      Iterates through each button and checks if it is debounced by using input_check_debounced().
      If a debounced rising edge is detected, function returns that direction. Otherwise, it returns
      NONE.
...
enum snake_direction receive_debounced_input(void)
{
  ...
  // While an order of preference is present here, due to the rising edge detection of the buttons
  // and the latching nature once an input is received, it is rarely relevant.
...
/*--------------------------------------------------------------------------------------
  input_check_debounced()
    Description:
      Checks the given input pin and debounces the input. If the pin is high, it shifts and increments
      the associated given integer and then checks it against the debounce mask. If the pin is low, the
      integer is set to zero.
      The shift and increment method is used as it easily identifies consecutive high inputs and will not
      overflow to zero as simply incrementing an integer would.
      The DEBOUNCE_MASK value has a number of low bits in the most significant bits followed by all high
      bits for the remainder. This picks up a rising edge where the input has transitioned from low to
      high.
...
bool input_check_debounced(uint16_t * debounce_value, const int8_t pin)
{
  ...
  {
    // Set to zero is a low is detected to guarantee a series of high inputs will be preceded by low bits
    // For example 0b001010111111 would not be detected, but if each low bit clears the value then this will never occur
    *debounce_value = 0;
  }
  return false;
}
...
/*--------------------------------------------------------------------------------------
  is_opposite_direction()
    ...
    Used for determining if a debounced value is in opposition to the direction the snake was just travelling
